    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                @if ($doctors->isEmpty())
                    <p class="text-red-600">Belum ada data dokter. Silakan tambah dokter terlebih dahulu sebelum membuat jadwal.</p>
                    <a href="{{ route('doctors.create') }}" class="text-indigo-600 hover:underline">+ Tambah Dokter</a>
                @else
                    <form action="{{ route('schedules.store') }}" method="POST" class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="time" name="start_time" value="{{ old('start_time') }}" class="mt-1 block w-full border-gray-300 rounded" required>
                            </div>
                            <div>
                                <label class="block font-medium text-sm text-gray-700">Jam Selesai</label>
                                <input type="time" name="end_time" value="{{ old('end_time') }}" class="mt-1 block w-full border-gray-300 rounded" required>
                            </div>
                        </div>

                        <div>
                            <label class="block font-medium text-sm text-gray-700">Kuota Pasien per Sesi</label>
                            <input type="number" name="quota" min="1" value="{{ old('quota', 10) }}" class="mt-1 block w-full border-gray-300 rounded" required>
                        </div>
